...
export async function getCurrentStudyPlan(uid: string): Promise<StudyPlan | null> {
...
export async function completeDay(uid: string, day: number, currentCompletedDays: number[]) {
  const completedDays = Array.from(new Set([...currentCompletedDays, day])).sort((a, b) => a - b)
